...
from core.libraries.investpy.data.funds_data import (
    fund_countries_as_list,
    funds_as_df,
    funds_as_dict,
    funds_as_list,
)
from core.libraries.investpy.resources.resources import get_resources_by_file_name
from core.libraries.investpy.utils.data import Data
from core.libraries.investpy.utils.extra import random_user_agent


def get_funds(country=None):
    """
    This function retrieves all the available `clients_funds` from Investing.com and returns them as a :obj:`pandas.DataFrame`,
    which contains not just the fund names, but all the fields contained on the `clients_funds.csv` file.
    All the available clients_funds can be found at: https://www.investing.com/funds/

    Args:
        country (:obj:`str`, optional): name of the country to retrieve all its available clients_funds from.

    Returns:
        :obj:`pandas.DataFrame` - funds_df:
            The resulting :obj:`pandas.DataFrame` contains all the clients_funds basic information retrieved from Investing.com,
            some of which is not useful for the user, but for the inner package functions, such as the `deposit_id` field,
            for example.
    ...
    Raises:
        ...
        FileNotFoundError: raised when the `clients_funds.csv` file was not found.
        IOError: raised if the `clients_funds.csv` file is missing or errored.
...
def get_funds_list(country=None):
    """
    This function retrieves all the available clients_funds and returns a list of each one of them.
    All the available clients_funds can be found at: https://www.investing.com/funds/
...
def get_funds_dict(country=None, columns=None, as_json=False):
    """
    This function retrieves all the available clients_funds on Investing.com and returns them as a :obj:`dict` containing
    the country, name, symbol, tag, deposit_id, issuer, isin, asset_class, currency and underlying data. All the available
    clients_funds can be found at: https://www.investing.com/funds/
...
def get_fund_countries():
    """
    This function retrieves all the country names indexed in Investing.com with available clients_funds to retrieve data
    ...
    Returns:
        :obj:`list` - countries:
            The resulting :obj:`list` contains all the available countries with clients_funds as indexed in Investing.com
...
def get_fund_recent_data(
    ...
    Raises:
        ...
        IOError: clients_funds object/file not found or unable to retrieve.
    ...
    Examples:
        >>> data = get_fund_recent_data(fund='bbva multiactivo conservador pp', country='spain')
        >>> data.head()
    ...
    funds = get_resources_by_file_name("clients_funds.csv")

    if funds is None:
        raise IOError("ERR#0005: clients_funds object not found or unable to retrieve.")
    ...
    id_ = funds.loc[(funds["name"].apply(unidecode).str.lower() == fund).idxmax(), "deposit_id"]
    name = funds.loc[
...
def get_fund_historical_data(
    ...
    Examples:
        >>> data = get_fund_historical_data(fund='bbva multiactivo conservador pp', country='spain', from_date='01/01/2010', to_date='01/01/2019')
        >>> data.head()
                     Open   High    Low  Close Currency
        Date
...
def get_funds_overview(country, as_json=False, n_results=100):
    """
    This function retrieves an overview containing all the real time data available for the main clients_funds from a country,
    such as the names, symbols, current value, etc. as indexed in Investing.com. So on, the main usage of this
    function is to get an overview on the main clients_funds from a country, so to get a general view. Note that since
    this function is retrieving a lot of information at once, by default just the overview of the Top 100 clients_funds
    is being retrieved, but an additional parameter called n_results can be specified so to retrieve N results.

    Args:
        country (:obj:`str`): name of the country to retrieve the clients_funds overview from.
        as_json (:obj:`bool`, optional):
            optional argument to determine the format of the output data (:obj:`pandas.DataFrame` or :obj:`json`).
    ...
    Raises:
        ...
        FileNotFoundError: raised when `clients_funds.csv` file is missing.
    ...
    root_ = fromstring(req.text)
    table = root_.xpath(".//table[@deposit_id='etfs']/tbody/tr")
...
def search_funds(by, value):
    """
    This function searches clients_funds by the introduced value for the specified field. This means that this function
    is going to search if there is a value that matches the introduced value for the specified field which is the
    `clients_funds.csv` column name to search in. Available fields to search clients_funds are 'name', 'symbol', 'issuer' and 'isin'.
    ...
    Raises:
        ...
        FileNotFoundError: raised if `clients_funds.csv` file is missing.
        IOError: raised if data could not be retrieved due to file error.
    ...
    funds.drop(columns=["tag", "deposit_id"], inplace=True)
