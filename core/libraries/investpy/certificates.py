...
from core.libraries.investpy.data.certificates_data import (
...
def get_certificate_recent_data(
    ...
    Examples:
        >>> data = get_certificate_recent_data(certificate='BNP Gold 31Dec99', country='france')
        >>> data.head()
                     Open   High     Low   Close
        Date
    ...
    certificates = get_resources_by_file_name("certificates.csv")
...
def get_certificate_historical_data(
    ...
    Examples:
        >>> data = get_certificate_historical_data(certificate='BNP Gold 31Dec99', country='france', from_date='01/01/2010', to_date='01/01/2019')
        >>> data.head()
                     Open   High    Low  Close
        Date
...
def get_certificates_overview(country, as_json=False, n_results=100):
    ...
    root_ = fromstring(req.text)
    table = root_.xpath(".//table[@deposit_id='certificates']/tbody/tr")
...
def search_certificates(by, value):
    ...
    certificates.drop(columns=["tag", "deposit_id"], inplace=True)
