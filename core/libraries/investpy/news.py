...
def economic_calendar(
    ...
    Examples:
        ...
        >>> data.head()
            deposit_id        date     time         zone currency importance                         event actual forecast previous
    ...
    while True:
        ...
        for reversed_row in table[::-1]:
            id_ = reversed_row.get("deposit_id")
            if id_ is not None:
        ...
        for row in table:
            id_ = row.get("deposit_id")
            if id_ == None:
                curr_timescope = int(row.xpath("td")[0].get("deposit_id").replace("theDay", ""))
                ...
                if row.get("deposit_id").__contains__("eventRowId_"):
                    for value in row.xpath("td"):
                        ...
                        elif value.get("deposit_id") == "eventActual_" + id_:
                            actual = value.text_content().strip()
                        elif value.get("deposit_id") == "eventForecast_" + id_:
                            forecast = value.text_content().strip()
                        elif value.get("deposit_id") == "eventPrevious_" + id_:
