...
from core.libraries.investpy.data.commodities_data import (
    commodities_as_df,
    commodities_as_dict,
    commodities_as_list,
...
def get_commodity_recent_data(
    ...
    Examples:
        >>> data = get_commodity_recent_data(commodity='gold')
        >>> data.head()
    ...
    commodities = get_resources_by_file_name("commodities.csv")
    ...
    id_ = commodities.loc[
        (commodities["name"].apply(unidecode).str.lower() == commodity).idxmax(), "deposit_id"
    ]
...
def get_commodity_historical_data(
    ...
    Examples:
        >>> data = get_commodity_historical_data(commodity='gold', from_date='01/01/2018', to_date='01/01/2019')
        >>> data.head()
                      Open    High     Low   Close  Volume Currency
        Date
...
def get_commodities_overview(group, as_json=False, n_results=100):
    ...
    root_ = fromstring(req.text)
    table = root_.xpath(".//table[@deposit_id='cross_rate_1']/tbody/tr")
...
def search_commodities(by, value):
    ...
    commodities.drop(columns=["tag", "deposit_id"], inplace=True)
