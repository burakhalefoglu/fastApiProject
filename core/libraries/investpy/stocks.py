...
from core.libraries.investpy.data.stocks_data import (
    stocks_as_df,
    stocks_as_dict,
    stocks_as_list,
)
...
def get_stock_recent_data(
    ...
    Examples:
        >>> data = get_stock_recent_data(stock='bbva', country='spain')
        >>> data.head()
    ...
    """
    stocks = get_resources_by_file_name("stocks.csv")
...
def get_stock_historical_data(
    ...
    Examples:
        >>> data = get_stock_historical_data(stock='bbva', country='spain', from_date='01/01/2010', to_date='01/01/2019')
        >>> data.head()
                     Open   High    Low  Close  Volume Currency
        Date
...
def get_stock_company_profile(stock, country="spain", language="english"):
    ...
    Examples:
        >>> company_profile = get_stock_company_profile(stock='bbva', country='spain', language='english')
        >>> company_profile
        company_profile = {
            url: 'https://www.investing.com/equities/bbva-company-profile',
            desc: 'Banco Bilbao Vizcaya Argentaria, S.A. (BBVA) is a ...'
...
def get_stock_financial_summary(
    ...
    Examples:
        >>> data = get_stock_financial_summary(stock='AAPL', country='United States', summary_type='income_statement', period='annual')
        >>> data.head()
