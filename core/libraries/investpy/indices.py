...
from core.libraries.investpy.data.indices_data import (
...
def get_index_recent_data(
    ...
    Examples:
        >>> data = get_index_recent_data(index='ibex 35', country='spain')
        >>> data.head()
    ...
    indices = get_resources_by_file_name("indices.csv")
    ...
    id_ = indices.loc[
        (indices["name"].apply(unidecode).str.lower() == index).idxmax(), "deposit_id"
    ]
    name = indices.loc[
...
def get_index_historical_data(
    ...
    Examples:
        >>> data = get_index_historical_data(index='ibex 35', country='spain', from_date='01/01/2018', to_date='01/01/2019')
        >>> data.head()
                       Open     High      Low    Close    Volume Currency
        Date
...
def search_indices(by, value):
    ...
    indices.drop(columns=["tag", "deposit_id"], inplace=True)
