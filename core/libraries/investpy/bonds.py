...
from core.libraries.investpy.data.bonds_data import (
...
def get_bond_recent_data(bond, as_json=False, order="ascending", interval="Daily"):
    ...
    Examples:
        >>> data = get_bond_recent_data(bond='Argentina 3Y')
        >>> data.head()
                      Open    High     Low   Close
        Date
    ...
    bonds = get_resources_by_file_name("bonds.csv")
    ...
    id_ = bonds.loc[(bonds["name"].apply(unidecode).str.lower() == bond).idxmax(), "deposit_id"]
    name = bonds.loc[
...
def get_bond_historical_data(
    ...
    Examples:
        >>> data = get_bond_historical_data(bond='Argentina 3Y', from_date='01/01/2010', to_date='01/01/2019')
        >>> data.head()
                    Open  High   Low  Close
        Date
...
def search_bonds(by, value):
    ...
    bonds.drop(columns=["tag", "deposit_id"], inplace=True)
