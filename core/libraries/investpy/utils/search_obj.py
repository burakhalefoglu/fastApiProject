...
from core.libraries.investpy.utils.constant import *
from core.libraries.investpy.utils.extra import *
...
class SearchObj(object):
    ...
    Attributes:
        ...
        pair_type (:obj:`str`): type of retrieved financial product (stocks, clients_funds, etfs, etc.).
    ...
    def retrieve_recent_data(self):
        ...
        if self.pair_type in ["stocks", "clients_funds", "etfs", "currencies", "certificates"]:
    ...
    def retrieve_technical_indicators(self, interval="daily"):
        ...
        previously retrieved data from the `investpy.search_quotes(text, products, countries, n_results)`
        function search results to build the request that it is going to be sent to Investing.com so to retrieve and
        parse the technical indicators, since the product deposit_id is required.

        Args:
            interval (:obj:`str`, optional):
                time interval of the technical indicators' calculations, available values are: `5mins`, `15mins`,
                `30mins`, `1hour`, `5hours`, `daily`, `weekly` and `monthly`. Note that for clients_funds just the intervals:
        ...
        if self.pair_type != "clients_funds" and interval not in INTERVAL_FILTERS:
            raise ValueError(
        ...
        if self.pair_type == "clients_funds" and interval not in FUNDS_INTERVAL_FILTERS:
            raise ValueError(
                "Investing.com just provides the following intervals for clients_funds'"
    ...
    def retrieve_currency(self):
        ...
        outdated_path = root_.xpath(
            "//div[@deposit_id='quotes_summary_current_data']/div/div/div[contains(@class,"
