...
from core.libraries.investpy.utils.extra import random_user_agent, resource_to_data
...
def technical_indicators(name, country, product_type, interval="daily"):
    ...
    available (stocks, clients_funds, etfs, indices, currency crosses, bonds, certificates and commodities) for different
    ...
    Examples:
        >>> data = technical_indicators(name='bbva', country='spain', product_type='stock', interval='daily')
        >>> data.head()
            technical_indicator    value           signal
    ...
    product_id = data.loc[
        (data[check].apply(unidecode).str.lower() == name).idxmax(), "deposit_id"
    ]
...
def moving_averages(name, country, product_type, interval="daily"):
    ...
    Examples:
        >>> data = moving_averages(name='bbva', country='spain', product_type='stock', interval='daily')
        >>> data.head()
          period  sma_value sma_signal  ema_value ema_signal
...
def pivot_points(name, country, product_type, interval="daily"):
    ...
    Examples:
        >>> data = pivot_points(name='bbva', country='spain', product_type='stock', interval='daily')
        >>> data.head()
                name     s3     s2     s1  pivot_points     r1     r2     r3
