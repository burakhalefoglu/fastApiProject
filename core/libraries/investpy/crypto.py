...
from core.libraries.investpy.data.crypto_data import cryptos_as_df, cryptos_as_dict, cryptos_as_list
...
def get_crypto_recent_data(crypto, as_json=False, order="ascending", interval="Daily"):
    ...
    Examples:
        >>> data = get_crypto_recent_data(crypto='bitcoin')
        >>> data.head()
    ...
    cryptos = get_resources_by_file_name("cryptos.csv")
    ...
    crypto_id = cryptos.loc[
        (cryptos["name"].apply(unidecode).str.lower() == crypto).idxmax(), "deposit_id"
    ]
...
def get_crypto_historical_data(
    ...
    Examples:
        >>> data = get_crypto_historical_data(crypto='bitcoin', from_date='01/01/2018', to_date='01/01/2019')
        >>> data.head()
                       Open     High      Low    Close  Volume Currency
        Date
...
def search_cryptos(by, value):
    ...
    cryptos.drop(columns=["tag", "deposit_id"], inplace=True)
